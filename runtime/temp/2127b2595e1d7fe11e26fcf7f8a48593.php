...
<div class="alert alert-<?php echo htmlentities((isset($tips_type) && ($tips_type !== '')?$tips_type:'info')); ?> alert-dismissable">
    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
    <p><?php echo (isset($page_tips) && ($page_tips !== '')?$page_tips:''); ?></p>
</div>
<?php endif; ?>
<div class="row">
    <div class="col-md-12">
        <div class="block">
            <?php if(!(empty($tab_nav['tab_list']) || (($tab_nav['tab_list'] instanceof \think\Collection || $tab_nav['tab_list'] instanceof \think\Paginator ) && $tab_nav['tab_list']->isEmpty()))): ?>
            <ul class="nav nav-tabs">
                <?php if(is_array($tab_nav['tab_list']) || $tab_nav['tab_list'] instanceof \think\Collection || $tab_nav['tab_list'] instanceof \think\Paginator): $i = 0; $__LIST__ = $tab_nav['tab_list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
            ...
            <div class="block-header bg-gray-lighter">
                ...
                <h3 class="block-title"><?php echo htmlentities($page_title); ?></h3>
            </div>
            <?php endif; ?>
            ...
            <form name="form-builder" id="form" class="form-horizontal form-builder">
                ...
                <input type="hidden" name="tab_name" value="<?php echo htmlentities((isset($tab_name) && ($tab_name !== '')?$tab_name:'')); ?>">
                <div class="block-content tab-content">
                    <div class="tab-pane fade in active push-20">
                        ...
                        <div class="block block-bordered">
                            <ul class="nav nav-tabs">
                                ...
                                <li <?php if($key == $tab): ?>class="active"<?php endif; ?>>
                                <a href="<?php echo htmlentities((isset($vo['url']) && ($vo['url'] !== '')?$vo['url']:'')); ?>"><?php echo htmlentities((isset($vo['title']) && ($vo['title'] !== '')?$vo['title']:'')); ?></a>
                                </li>
                                <?php endforeach; endif; else: echo "" ;endif; ?>
                            </ul>
                            <div class="block-content tab-content">
                                <div class="tab-pane active" id="btabs-static-home">
                                    <div id="jstree" class="push"><?php echo (isset($nodes) && ($nodes !== '')?$nodes:''); ?></div>
                                </div>
                            </div>
                        </div>
                        <?php else: ?>
                    ...
                    <div class="push-20">
                        ...
                        <?php endif; ?>
                        <a class="btn btn-default" href="<?php echo url('index'); ?>">
                            返回
                        </a>
                    </div>
                </div>
...
<script>
    $(document).ready(function(){
        ...
        $('#form').submit(function () {
            ...
            $.post('<?php echo url("access", ["module" => $module, "uid" => $uid, "tag" => $tab]); ?>', {nodes:nodes}).success(function (res) {
                if (res.code) {
