...
class Project extends Admin
{
    ...
    public function index()
    {
        ...
        //分页数组
        $nav = Db::table("dp_admin_navigation")->where(['pid'=>20])->order('sort')->field(['id','title'])->select();
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($page = ProjectModel::create($data)) {
                // 记录行为
                action_log('project_add', 'admin_project', $page['id'], UID, "添加");
        ...
        //所属分页
        $list = Db::table("dp_admin_navigation")->where(['pid'=>20])->order('sort')->field(['id','title'])->select();
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (ProjectModel::update($data)) {
                // 记录行为
                action_log('project_edit', 'admin_project', $id, UID, "编辑");
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        $page_title = ProjectModel::where('id', 'in', $ids)->column('title');
        return parent::setStatus($type, ['page_'.$type, 'admin_project', 0, UID, implode('、', $page_title)]);
    }
    ...
    public function quickEdit($record = [])
    {
        ...
        $page    = ProjectModel::where('id', $id)->value($field);
        $details = '字段(' . $field . ')，原值(' . $page . ')，新值：(' . $value . ')';
        return parent::quickEdit(['project_edit', 'admin_project', $id, UID, $details]);
    }
}
